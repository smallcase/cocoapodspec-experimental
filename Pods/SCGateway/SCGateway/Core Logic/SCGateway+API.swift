...
/// Smallcase Transaction Methods
 public extension SCGateway {
    ...
    guard Config.gatewayName != nil else {
    ...
    guard Config.gatewayName != nil else { completion(nil, SCGatewayError.configNotSet)
        return
    }
...
//
//    func getUserInvestments(iscids: [String]?, completion: @escaping(Result<Data, Error>) -> Void) {
//        guard Config.gatewayName != nil else {
//
//            completion(.failure(SCGatewayError.configNotSet))
//            return
//        }
//        guard Config.broker != nil, let userStatus = Config.userStatus, userStatus == .connected else {
//             completion(.failure(SCGatewayError.uninitialized))
//            return
//        }
//
...
//    }
    ...
    @objc func getUserInvestmentDetails(iscid: String, completion: @escaping(Data?, Error?) -> Void ) {
        ...
        sessionProvider.requestJson(service: GatewayService.getUserInvestments(iscids: [iscid]) ) { (result) in
            switch result {
            case .success(let data):
                completion(data, nil)
            case .failure(let error):
