...
protocol ViewStateComponentDelegate: AnyObject {
    func onClickCancel()
}
/**
...
class TransactionCompletionStatusView: UIView {
    ...
    fileprivate var timerString: Int = -1 {
        didSet {
            ...
            redirectLabel.text = "Redirecting you to \(SessionManager.gatewayName!) in \(timerString)"
            if timerString == 0  {
                timer.invalidate()
    ...
    func setupButton() {
       ...
       ctaButton.titleLabel?.font =  UIFont(name: "GraphikApp-Medium", size: 15 )
        ...
        case .orderInQueue:
            ...
            ctaButton.tintColor = ScgatewayColor.linkBlue
            ctaButton.backgroundColor = .clear
            ctaButton.setTitleColor(ScgatewayColor.linkBlue, for: .normal)
